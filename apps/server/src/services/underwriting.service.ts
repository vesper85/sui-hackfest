...
class UnderwritingService {
  ...
  calculateCollateralScore(
    ...
    const verificationScore =
      (verifiedCount / collateralAssetsList.length) * 100;
  ...
  async generateUnderwritingReport(submissionId: string): Promise<string> {
    ...
    const maxLoanAmount =
      totalCollateralValue > 0
        ? this.calculateMaxLoanAmount(totalCollateralValue, riskTier)
        : 0;
    const recommendedInterestRate = this.calculateInterestRate(riskTier);
    const recommendedLtv =
      totalCollateralValue > 0
        ? Math.min(maxLoanAmount / totalCollateralValue, 0.8)
        : 0;

    const [latestReport] = await db
      .select({ reportVersion: underwritingReports.reportVersion })
    ...
    const nextVersion = (latestReport?.reportVersion ?? 0) + 1;
    ...
    const [report] = await db
      ...
      .values({
        ...
        probabilityScore:
          probabilityAssessment.overallProbabilityScore.toString(),
  ...
  private aggregateFinancialMetrics(extractedData: any[]): {
    ...
    const debtToIncomeRatio =
      monthlyIncome > 0
        ? (monthlyExpenses + totalLiabilities / 12) / monthlyIncome
        : 0;
